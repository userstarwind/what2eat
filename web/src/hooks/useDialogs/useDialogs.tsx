  ...
  return (
    <Dialog
      ...
      slotProps={{
        paper: {
          component: 'form',
          onSubmit: async (event: React.SyntheticEvent<HTMLDivElement, SubmitEvent>) => {
...
export function useDialogs(): DialogHook {
  ...
  const alert = useEventCallback<OpenAlertDialog>(
    (msg: React.ReactNode, options: AlertOptions = {}) => {
      const { onClose, ...rest } = options;
      return open(AlertDialog, { ...rest, msg }, { onClose });
    },
  );

  const confirm = useEventCallback<OpenConfirmDialog>(
    (msg: React.ReactNode, options: ConfirmOptions = {}) => {
      const { onClose, ...rest } = options;
      return open(ConfirmDialog, { ...rest, msg }, { onClose });
    },
  );

  const prompt = useEventCallback<OpenPromptDialog>(
    (msg: React.ReactNode, options: PromptOptions = {}) => {
      const { onClose, ...rest } = options;
      return open(PromptDialog, { ...rest, msg }, { onClose });
    },
  );
